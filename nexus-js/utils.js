...
// 🔹 Глобальная константа API (автоматически подстраивается под хост)
window.NEXUS_API = window.location.origin;
...
export function initTheme() {
  const saved = localStorage.getItem("nexus_theme") || "hellokitty";
  applyTheme(saved);
}
...
// ═══════════════════════════════════════════════════════
// MODAL HELPERS — добавлены в utils.js
// ═══════════════════════════════════════════════════════
export function showModal(modal, options = {}) {
  ...
  document
    .querySelectorAll(".nexus-modal.animating")
    .forEach((m) => m.classList.remove("animating"));
  modalEl.classList.add("active");
  void modalEl.offsetWidth;
  modalEl.classList.add("animating");
  setTimeout(() => {
    modalEl.classList.remove("animating");
    if (focusSelector) modalEl.querySelector(focusSelector)?.focus();
  }, duration);
}
