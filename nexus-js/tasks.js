...
function renderFileItem(f, type, parentId, taskId) {
  ...
  // 🔥 ИСПРАВЛЕНО: динамический URL для файлов
  const url =
    f.url ||
    `${API_BASE}/api/tasks/files/${window.nexusAuth?.user?.id}/${taskId}/main/${f.serverName}`;
...
function updateTaskStats(tasks) {
  ...
  animateValue(
    dashCritical,
    parseInt(dashCritical.textContent) || 0,
    critical,
    500,
  );
}
...
// ============================================================================
// 🔹 ИНИЦИАЛИЗАЦИЯ & СТАТИЧЕСКИЕ СЛУШАТЕЛИ
// ============================================================================
function setupEventListeners() {
  ...
  // Глобальный клик: добавление этапа
...
if (!window.__nexus_tasks_initialized) {
  window.__nexus_tasks_initialized = true;
