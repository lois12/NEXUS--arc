...
const NOTES_API = `${API_BASE}/api/media-plan/notes`;
const MEDIA_API = `${API_BASE}/api/mediaplan`;
...
export function openEventModal(ds) {
  ...
  apiGetNotes().then(() => {
    ...
    renderEventList(ds);
    showModal("event-modal", { focusSelector: "#evt-name" });
  });
}
...
function setupCalendarSwipe() {
  const calendarHeader = document.getElementById("cal-month-year"); // 🔥 Шапка календаря
  ...
  calendarHeader.addEventListener(
    "touchstart",
    () => {
      calendarHeader.classList.add("swipe-highlight");
    },
    { passive: true },
  );
  ...
  calendarHeader.addEventListener(
    "touchend",
    () => {
      setTimeout(() => calendarHeader.classList.remove("swipe-highlight"), 300);
    },
    { passive: true },
  );
}
...
function handleSwipeGesture() {
  ...
  const swipeThreshold = touchStartX - touchEndX;

  if (Math.abs(swipeThreshold) < minSwipeDistance) return; // 🔥 Слишком короткий свайп

  if (swipeThreshold > 0) {
    // Swipe Left -> Next Month
    document.getElementById("cal-next")?.click(); // 🔥 Используем существующую логику
  } else {
    // Swipe Right -> Prev Month
    document.getElementById("cal-prev")?.click(); // 🔥 Используем существующую логику
  }
}
...
function setupEventListeners() {
  ...
  dayEventsList?.addEventListener("click", async (e) => {
    ...
    if (delBtn) {
      ...
      try {
        await fetch(`${MEDIA_API}/event/${date}/${ev}/${file}`, {
          method: "DELETE",
          headers: { Authorization: `Bearer ${window.nexusAuth?.token}` },
        });
  ...
  document
    ...
    ?.addEventListener("click", () => hideModal("logs-modal"));
  logsModal?.addEventListener("click", (e) => {
    if (e.target === logsModal) hideModal("logs-modal");
  });
...
export function initMediaplan() {
  setupEventListeners();
  setupCalendarSwipe(); // 🔥 Подключаем свайп
  console.log("📅 Mediaplan module initialized");
}

// 🔥 ЗАЩИТА ОТ ДВОЙНОГО ЗАПУСКА
if (!window.__nexus_mediaplan_initialized) {
  window.__nexus_mediaplan_initialized = true;
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initMediaplan);
