...
const app = express();
const server = http.createServer(app); // Для WebSocket
app.use(express.json({ limit: "10mb" }));
app.use(express.urlencoded({ extended: true, limit: "10mb" }));
// 🔹 CORS: разрешаем запросы с любого IP в локальной сети + localhost
app.use((req, res, next) => {
  res.header("Access-Control-Allow-Origin", "*"); // 🔥 Для тестов. В продакшене укажи конкретный домен
  res.header(
...
wss.on("connection", (ws, req, user) => {
  console.log(
    `[WS] User ${user.username || user.userId} connected to log stream.`,
  );

  ws.send(
    JSON.stringify({
      level: "info",
      message: `[WS] Authenticated as ${user.username || user.userId}`,
    }),
  );
  ...
  // 🔥 ПЕРЕХВАТ ЛОГОВ (ТОЛЬКО ДЛЯ РАЗРАБОТКИ!)
  ...
  ws.on("close", () => {
    console.log(`[WS] User ${user.username || user.userId} disconnected.`);
    // Восстанавливаем функции (опционально)
...
console.log("[WS] WebSocket listener configured for upgrades.");
...
async function readJSON(filePath) {
  try {
    ...
    if (e.code === "ENOENT") {
      // 🔥 ФАЙЛА НЕТ - создаём папку и возвращаем пустой массив
      const dir = path.dirname(filePath);
      await fs.mkdir(dir, { recursive: true }); // 🔥 Рекурсивно создаст всю цепочку папок
      return []; // 🔥 Возвращаем пустой массив для новых пользователей
    }
    throw e;
  }
}
...
function createFileFilter(types) {
  ...
  return (req, file, cb) => {
    ...
    console.log(`📎 [MULTER CHECK] File: ${file.originalname} | Ext: ${ext}`);

    const allowed = types.some((t) => a[t]?.includes(ext));
    if (allowed) {
      cb(null, true);
    } else {
      console.warn(`⛔ [MULTER BLOCKED] ${ext} not in allowed list`);
      cb(new Error(`Недопустимый формат: ${ext}`), false);
    }
  };
}
...
const docsStor = multer.diskStorage({
  destination: async (req, file, cb) => {
    try {
      const folder = sanitizeFolder(req.params.folder);
      const dir = path.join(DATA_DIR, "documents", folder);
      await fs.mkdir(dir, { recursive: true });
      cb(null, dir);
    } catch (e) {
      cb(e);
...
const docsUp = multer({
  storage: docsStor,
  fileFilter: createFileFilter(["images", "docs"]), // Разрешаем картинки + документы
  limits: { fileSize: 25 * 1024 * 1024 }, // 25МБ
});
...
// ============================================================================
// 🔹 5.8 НОВОЕ: МОДУЛЬ ДОКУМЕНТЫ (DATA/DOCUMENTS)
// ============================================================================

// 🔹 Базовая папка для документов
const DOCS_ROOT = path.join(DATA_DIR, "documents");

// 🔹 Утилита: получить безопасный путь к папке документов
async function getDocsFolder(folder) {
  const safeFolder = sanitizeFolder(folder);
  const folderPath = path.join(DOCS_ROOT, safeFolder);
  await fs.mkdir(folderPath, { recursive: true });
  return { safeFolder, folderPath };
}

// 🔹 Утилита: получить список файлов с метаданными
async function listDocsFiles(folderPath) {
  try {
    const entries = await fs.readdir(folderPath, { withFileTypes: true });
    const files = [];
    for (const e of entries) {
      if (e.isFile()) {
        const stat = await fs.stat(path.join(folderPath, e.name));
        files.push({
          name: e.name.replace(/^\d+-/, ""), // убираем префикс даты
          serverName: e.name,
          size: stat.size,
          modified: stat.mtime,
...
app.get("/api/documents/:folder", authMiddleware, async (req, res) => {
  if (res.headersSent) return;
  try {
    const { safeFolder, folderPath } = await getDocsFolder(req.params.folder);
    const files = await listDocsFiles(folderPath);
    res.json(files);
  } catch (e) {
    console.error("[DOCS LIST ERR]", e);
    if (!res.headersSent)
      res.status(500).json({ error: "Ошибка чтения папки" });
  }
});

// 📤 Загрузка документов в папку
app.post(
  "/api/documents/:folder/upload",
  authMiddleware,
  docsUp.array("files", 50), // 🔹 используем тот же storageUp из секции 5
  handleMulter,
  async (req, res) => {
    if (res.headersSent) return;
    if (!req.files?.length)
      return res.status(400).json({ error: "Нет файлов" });
    try {
      const { safeFolder } = await getDocsFolder(req.params.folder);
      res.json(
        req.files.map((f) => ({
          name: f.originalname,
          serverName: f.filename,
          url: `/api/documents/${safeFolder}/${f.filename}`,
          size: f.size,
        })),
      );
    } catch (e) {
...
app.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error("[DOCS DELETE ERR]", e);
      if (!res.headersSent) res.status(500).json({ error: "Ошибка удаления" });
    }
  },
);

// 🗂️ Удаление всей папки документов (вкладки)
app.delete(
  "/api/documents/folder/:folder",
  authMiddleware,
  async (req, res) => {
    if (res.headersSent) return;
    try {
      ...
      const protectedFolders = ["general", "contracts", "reports", "templates"];
      if (protectedFolders.includes(safeFolder)) {
        return res.status(403).json({ error: "Системная папка" });
      }
      ...
      await logFileAction(
        ...
        `documents/${safeFolder}`,
      );
      res.json({ success: true });
    } catch (e) {
      console.error("[DOCS FOLDER DELETE ERR]", e);
      if (!res.headersSent)
        res.status(500).json({ error: "Ошибка удаления папки" });
    }
  },
...
// ⬇️ Скачивание документа (публичный доступ, как у storage)
...
app.get(
  "/api/documents/:folder/download-all",
  authMiddleware,
  async (req, res) => {
    if (res.headersSent) return;
    try {
      ...
      const archiveName = `nexus-docs-${safeFolder}.zip`;
      res.setHeader("Content-Type", "application/zip");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="${encodeURIComponent(archiveName)}"`,
      ...
      archive.on("error", (err) => {
        console.error("[DOCS ARCHIVE ERR]", err);
        if (!res.headersSent) res.status(500).end();
      });
      ...
      await logFileAction(req, "СКАЧИВАНИЕ АРХИВА ДОКУМЕНТОВ", archiveName);
    } catch (e) {
      console.error("[DOCS ARCHIVE ERR]", e);
      if (!res.headersSent)
        res.status(500).json({ error: "Ошибка архива: " + e.message });
    }
  },
...
// 🔹 НОВОЕ: API для получения системных ресурсов (CPU, RAM, DISK)
app.get("/api/admin/system-stats", adminMiddleware, async (req, res) => {
  ...
  try {
    ...
    const disk = Math.round((usedDisk / totalDisk) * 100);
    ...
    console.error("[SYSTEM STATS ERR]", e);
    if (!res.headersSent) res.status(500).json({ error: e.message });
  }
});
...
// 🔹 НОВОЕ: Получение системных ресурсов
app.get("/api/admin/system-stats", adminMiddleware, async (req, res) => {
  if (res.headersSent) return;
  try {
    const si = require("systeminformation");
...
(async function init() {
  ...
  // ============================================================================
  // 🔹 WEBSOCKET ЛОГ-АГРЕГАТОР (вставьте перед app.listen())
  // ============================================================================
  const WebSocket = require("ws");
  const wss = new WebSocket.Server({ noServer: true });
  ...
  wss.on("connection", (ws, req, user) => {
    ...
    ws.send(
      JSON.stringify({
        level: "info",
        message: `[WS] Connected as ${user.username || user.userId}`,
      }),
    );
  ...
  server.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `🌐 NEXUS LOCAL: http://192.168.31.5:${PORT}`,
      `💬 WS: ws://192.168.31.5:${PORT}?token=<token>`,
    );
    console.log(
